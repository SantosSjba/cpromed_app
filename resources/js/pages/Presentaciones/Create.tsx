...
export default function PresentacionesCreate({
    medicamentos,
    ubicaciones,
}: {
    ...
    return (
        <AppLayout title="Nueva presentación">
                ...
                <form onSubmit={submit} className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" min={0} value={data.precio_compra === '' ? '' : data.precio_compra} onChange={(e) => setData('precio_compra', e.target.value === '' ? '' : Number(e.target.value))} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                        </div>
                        <div>
                            <label className="mb-1 block text-sm font-medium text-slate-700">Precio venta</label>
                            <input type="number" step="0.01" min={0} value={data.precio_venta === '' ? '' : data.precio_venta} onChange={(e) => setData('precio_venta', e.target.value === '' ? '' : Number(e.target.value))} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
                        </div>
                    </div>
                    <div>
